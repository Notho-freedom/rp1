...
class hanta(tk.Tk):
    ...
    def __init__(self, root):
        ...
        def D_T():
            ...
            self.tach.set(0)
            heur=now.strftime("%H:%M:%S")
            tk.Label(f_g,text=("Heure("+heur+")"),fg="white",bg="#41b77f",font=("times new roman",10,"bold"),bd=0).place(x=200,y=420)
            heur=now.strftime("%H%M%S")
        ...
        #bd
        f_gd= Frame(self.root, width=350,height=700,bg="black",bd=4).place(x=1020,y=80)

        def bar():

            self.progress = ttk.Progressbar(f_gd,length=200,mode='determinate')
            self.progress.place(x=1110,y=100)

            labell=Label(f_gd,font=("times new roman",10,"bold"),bg="black",fg="red")
            labell.place(x=1323,y=100,width=35,height=23)
            ...
            def start():
                ...
                for i in range(1,101,1):
                    ...
                    labell.config(text=str(i)+"%")
                    time.sleep(0.01)
                self.progress['value']=100

            self.button = ttk.Button(f_gd,text="start", command=start)
            self.button.place(x=1021,y=160)
        bar()
        ...
        def get_cursor():
            ...
            self.tr.set(row[3])
            self.DD.set(row[4])
            self.heure.set(row[5])
            self.Addresse.set(row[6])
            self.nser.set(row[7])
            self.Dett.set(row[8])
        self.st.bind('<ButtonRelease-1>',get_cursor)    
        #suite_framg
        Label(f_g,text="Date",fg="white",bg="#41b77f",font=("times new roman",11,"bold"),bd=2).place(x=7,y=420)
        ...
        Label(f_g,text="Addresse",fg="white",bg="#41b77f",font=("times new roman",11,"bold"),bd=2).place(x=7,y=575)
        Entry(f_g,textvariable=self.Addresse,font=("times new roman",10),bd=2).place(x=7,y=595)

        Label(f_g,text="Dette",fg="white",bg="#41b77f",font=("times new roman",11,"bold"),bd=2).place(x=200,y=575)
